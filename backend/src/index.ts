...
class ServerManager {
  ...
  private initializeSSL(): void {
    ...
    try {
      ...
      if (!fs.existsSync(keyPath) || !fs.existsSync(certPath)) {
        ...
        logger.warn('   mkcert -key-file .cert/localhost-key.pem -cert-file .cert/localhost-cert.pem localhost 127.0.0.1 localhost ::1');
  ...
  private logStartupInfo(): void {
    ...
    if (this.useHttps) {
      ...
      console.log('   mkcert -key-file .cert/localhost-key.pem -cert-file .cert/localhost-cert.pem localhost 127.0.0.1 localhost ::1');
    }
 ...
 *    mkcert -key-file .cert/localhost-key.pem -cert-file .cert/localhost-cert.pem localhost 127.0.0.1 localhost ::1
 *
 * 2. 環境変数設定(.env):
 *    PORT=8000
 *    HTTPS_PORT=8443
 ...
 *
 * 【アクセスURL】
 * - HTTPS(推奨): https://localhost:8443/api/v1
 * - HTTP(自動リダイレクト): http://localhost:8000/api/v1
 */
