...
router.delete('/master/:table/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    if (table === 'locations') {
      ...
      // inspection_item_results.inspection_item_id → inspection_items を参照しているため先に削除
      await tx.$executeRawUnsafe(`DELETE FROM inspection_item_results WHERE inspection_item_id = $1::uuid`, id);
    }
...
router.post('/master/bulk-delete', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    if (table === 'locations') {
      ...
      // inspection_item_results.inspection_item_id → inspection_items を参照しているため先に削除
      await tx.$executeRawUnsafe(`DELETE FROM inspection_item_results WHERE inspection_item_id = ANY($1::uuid[])`, ids);
    }
