...
const DuplicateLocationMerge: React.FC<DuplicateLocationMergeProps> = ({ isOpen, onClose, onMerged }) => {
  ...
  const fetchUsageStats = useCallback(async () => {
    try {
      ...
      if (res?.success && res?.data) {
        const list: Array<{ locationId: string } & LocationUsageStats> = res.data;
  ...
  const fetchNearby = useCallback(async () => {
    if (!baseLocation?.latitude || !baseLocation?.longitude) return;
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-amber-900">
                ...
                <p>
                  ...
                  統合元の場所は無効化され一覧から非表示になります（削除はされず、履歴として残ります）。
                </p>
              </div>
            </div>
            <ul className="text-xs text-amber-800 pl-7 list-disc space-y-0.5">
              {selectedCandidates.map(c => (
                <li key={c.location.id}>{c.location.name}（{c.distanceM}m）</li>
              ))}
            </ul>
          </div>
        )}
