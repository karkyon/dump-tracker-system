...
export const GPS_CONFIG = {
  ...
  // ✅ Fix-5B: タイムアウト（ミリ秒） 10000 → 15000
  // 理由: MAXIMUM_AGE=0 でリアルタイム取得を強制するため取得時間を延長
  TIMEOUT: 15000,
  
  // ✅ Fix-5A: キャッシュ最大使用期限（ミリ秒） 5000 → 0
  // 理由: 5秒前のキャッシュ座標が混入すると「停止中に移動した」と誤判定される
  //       MAXIMUM_AGE=0 で常にリアルタイム測位を強制する
  MAXIMUM_AGE: 0,
  
  // ✅ Fix-5C: 最小移動距離（メートル） 1 → 10
  // 理由: GPS自然揺れ(1〜5m)を全てカウントすると停止中でも距離が増加し続ける
  //       10m未満の移動はノイズとしてスキップ
