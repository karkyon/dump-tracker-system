...
router.get('/activity', authenticateToken(), requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  ...
  const logPath = path.join(process.cwd(), 'logs', 'combined.log');
  const stripAnsi = (s: string) => s.replace(/\u001b\[[0-9;]*m/g, '');
  const ACTION_KEYWORDS = ['認証成功', '認証失敗', 'ログイン', 'ログアウト', 'LOGIN', 'LOGOUT', 'AUTH', 'USER_ACTION', '運行開始', '運行終了', 'GPS', 'フィードバック'];
  try {
    const stat = fs.statSync(logPath);
    ...
    const activityLogs = content.split('\n').filter(Boolean)
      ...
      .map(l => {
        ...
        try {
          const d = JSON.parse(clean);
          return { timestamp: d.timestamp || '', level: (d.level || 'info').toUpperCase(), message: d.message || clean, data: d.data || d.frontendData || null };
        } catch {
...
router.get('/server-status', authenticateToken(), requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  ...
  // ディスク使用量 — split結果をString()で確実に文字列化してundefined回避
  let diskInfo = { total: '不明', used: '不明', free: '不明', usedPercent: '不明' };
  try {
    const dfParts: string[] = execSync('df -h / | tail -1').toString().trim().split(/\s+/);
    diskInfo = {
      total:       String(dfParts[1] || '?'),
      used:        String(dfParts[2] || '?'),
      free:        String(dfParts[3] || '?'),
      usedPercent: String(dfParts[4] || '?'),
    };
  } catch { /* デフォルト値を使用 */ }
  ...
  try {
    backendServiceStatus = execSync('systemctl is-active dump-tracker-backend.service 2>/dev/null', { timeout: 3000 }).toString().trim();
